...
## 短链生成逻辑
1. 通过从[a-zA-Z][0-9]共计62个字符，生成62^7≈3.5万亿的可用短链接。
...
2. **安装**：
   ...
   - 设置`application-[test/prod].yml中url相关属性
...
4. **运行系统**：
   - 项目配置完成后，通过maven将项目的打包为jar包，并启动即可
   ...
   - -Dspring.profiles.active=prod为正式环境，如果要部署测试环境，将prod改为test即可
   - 在 `http://www.a.com/easyurl/index.html` 访问系统，初始账号admin  密码 th749gds73phg4gg
   - 登录系统，在系统管理-开放账户，添加可用的appId/appSecret
5. **调用方式**：
   1. 通过http请求直接调用
      ```java
    ...
    2. 通过url-client配置
        1. 将[url-client-1.0.1.jar](https://github.com/justcode-cc/easyurl/releases/)添加到你的项目或者私服
        2. 在项目中引用
            ```java
            <dependency>
